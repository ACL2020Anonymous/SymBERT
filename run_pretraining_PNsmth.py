...
# Decompiled from: Python 2.7.5 (default, Mar 18 2019, 22:58:01) 
# [GCC 4.8.5 20150623 (Red Hat 4.8.5-28)]
# Embedded file name: run_pretraining.py
# Compiled at: 2019-07-04 06:17:10
"""Run masked LM/next sentence masked_lm pre-training for BERT."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import numpy
numpy.random.seed(8888)
import datetime, os, time, modeling, optimization, tensorflow as tf
...
flags.DEFINE_bool('load_adam_parameters', True, 'Whether to load Adam parameters')
...
flags.DEFINE_integer('save_checkpoints_steps', 4000, 'How often to save the model checkpoint.')
...
flags.DEFINE_integer('pre_train_steps', 0, 'Number of previous steps before rewarmup.')
flags.DEFINE_integer('rewarmup_steps', 0, 'Number of warmup steps.')
flags.DEFINE_bool('use_memory_ckpt', False, 'Whether to use gradient checkpoint to save memory')

class _LogSessionRunHook(tf.train.SessionRunHook):

    def __init__(self, global_batch_size, display_every=100, hvd_rank=-1):
    ...
    def after_run(self, run_context, run_values):
        ...
        if print_step == 1 or print_step % self.display_every == 0:
            ...
            if self.hvd_rank == 0:
                print('tpckl %s %6i %11.1f %10.4e %10.4e %6.3f     %6.4e' % (
                 str(datetime.datetime.now()), print_step, img_per_sec, mlm_loss, nsp_loss, total_loss, lr))
            self.elapsed_secs = 0.0
            self.count = 0


def model_fn_builder(bert_config, init_checkpoint, learning_rate, num_train_steps, num_warmup_steps, use_tpu, use_one_hot_embeddings, hvd=None):
    ...
    def model_fn(features, labels, mode, params):
        ...
        model = modeling.BertModel(config=bert_config, is_training=is_training, input_ids=input_ids, input_mask=input_mask, token_type_ids=segment_ids, use_one_hot_embeddings=use_one_hot_embeddings, compute_type=tf.float16 if FLAGS.use_fp16 else tf.float32)
        ...
        next_sentence_loss, next_sentence_example_loss, next_sentence_log_probs = get_next_sentence_output(bert_config, model.get_pooled_output(), next_sentence_labels, FLAGS.next_sent_type)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            train_op = optimization.create_optimizer(total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, hvd, FLAGS.use_fp16, FLAGS.pre_train_steps, FLAGS.rewarmup_steps, FLAGS.use_memory_ckpt)
        if not FLAGS.load_adam_parameters:
...
def get_next_sentence_output(bert_config, input_tensor, labels, num_classes):
    ...
    with tf.variable_scope('cls/seq_relationship'):
        label_weights = tf.constant(
          [[1,0,0],[0,1,0],[0,0,1],[0.05,0.9,0.05],[0.05,0.05,0.9]]
        )
        output_weights = tf.get_variable('output_weights', shape=[
         num_classes, bert_config.hidden_size], initializer=modeling.create_initializer(bert_config.initializer_range))
        output_bias = tf.get_variable('output_bias', shape=[num_classes], initializer=tf.zeros_initializer())
        logits = tf.matmul(input_tensor, output_weights, transpose_b=True)
        logits = tf.nn.bias_add(logits, output_bias)
        log_probs = tf.nn.log_softmax(logits, axis=-1)

        labels = tf.reshape(labels, [-1])
        smoothing_labels = tf.gather(label_weights, labels)
#        one_hot_labels = tf.one_hot(labels, depth=num_classes, dtype=tf.float32)
#        ones = tf.ones(log_probs.shape,dtype=tf.float32)
#        others = ones * (FLAGS.smoothing/(num_classes-1.0))
#        smoothing_labels = (1.0-one_hot_labels)*others + one_hot_labels*(1.0-FLAGS.smoothing)

        per_example_loss = -tf.reduce_sum(smoothing_labels * log_probs, axis=-1)
...
def input_fn_builder(input_files, max_seq_length, max_predictions_per_seq, is_training, num_cpu_threads=4, hvd=None):
...
def main(_):
    ...
    if FLAGS.horovod:
        ...
        hvd.init()
        tmp_outptu_dir = ('{0}-tmp-{1}').format(FLAGS.output_dir, hvd.local_rank())
        FLAGS.output_dir = FLAGS.output_dir if hvd.rank() == 0 else tmp_outptu_dir
    ...
    tf.logging.info(('*** Input Files ***: {0} files').format(len(input_files)))
...
#        rewriter_config_pb2.RewriterConfig.ON)
    ...
    run_config = tf.contrib.tpu.RunConfig(cluster=tpu_cluster_resolver, master=FLAGS.master, model_dir=FLAGS.output_dir, keep_checkpoint_max=1000 if not FLAGS.horovod or hvd.rank() == 0 else 1, save_checkpoints_steps=FLAGS.save_checkpoints_steps if not FLAGS.horovod or hvd.rank() == 0 else None, session_config=config, tpu_config=tf.contrib.tpu.TPUConfig(iterations_per_loop=FLAGS.iterations_per_loop, num_shards=FLAGS.num_tpu_cores, per_host_input_for_training=is_per_host), log_step_count_steps=10000 * FLAGS.iterations_per_loop if FLAGS.report_loss else 100 * FLAGS.iterations_per_loop)
    model_fn = model_fn_builder(bert_config=bert_config, init_checkpoint=FLAGS.init_checkpoint, learning_rate=FLAGS.learning_rate if not FLAGS.horovod else FLAGS.learning_rate * hvd.size(), num_train_steps=FLAGS.num_train_steps, num_warmup_steps=FLAGS.num_warmup_steps, use_tpu=FLAGS.use_tpu, use_one_hot_embeddings=FLAGS.use_tpu, hvd=None if not FLAGS.horovod else hvd)
    training_hooks = []
    ...
    if FLAGS.report_loss and (not FLAGS.horovod or hvd.rank() == 0):
        ...
        training_hooks.append(_LogSessionRunHook(global_batch_size, 100, -1 if not FLAGS.horovod else hvd.rank()))
    ...
    if FLAGS.do_train:
        ...
        train_input_fn = input_fn_builder(input_files=input_files, max_seq_length=FLAGS.max_seq_length, max_predictions_per_seq=FLAGS.max_predictions_per_seq, is_training=True, hvd=None if not FLAGS.horovod else hvd)
    ...
    if FLAGS.do_eval and (not FLAGS.horovod or hvd.rank() == 0):
        ...
        eval_input_fn = input_fn_builder(input_files=input_files, max_seq_length=FLAGS.max_seq_length, max_predictions_per_seq=FLAGS.max_predictions_per_seq, is_training=False, hvd=None if not FLAGS.horovod else hvd)
...
if __name__ == '__main__':
    ...
    tf.app.run()
# okay decompiling run_pretraining.pyc
